  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-6 duration-700">
      ...
      {prediction.image_url && (
        <div className="bg-gradient-to-b from-emerald-900/20 to-transparent border border-emerald-500/30 rounded-[2rem] overflow-hidden shadow-2xl">
          <div className="p-6 bg-black/40 backdrop-blur-sm">
            <div className="flex items-center gap-3 mb-4">
              <FiImage className="text-emerald-400 text-lg" />
              <h3 className="text-emerald-500 text-xs font-black uppercase tracking-[0.2em]">Scanned Crop Image</h3>
            </div>
            <img
              src={prediction.image_url}
              alt="Scanned crop"
              className="w-full max-w-md mx-auto rounded-xl shadow-lg border border-emerald-500/20 hover:border-emerald-500/50 transition-all"
            />
          </div>
        </div>
      )}
      ...
      {/* Prediction Chart - Comparison Graph */}
      {prediction.all_predictions && (
        <PredictionChart 
          predictions={prediction.all_predictions} 
          diseaseHighlight={prediction.disease_name}
        />
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {prediction.symptoms && (
          <DetailSection 
            ...
            content={Array.isArray(prediction.symptoms) ? prediction.symptoms.join('\n') : prediction.symptoms} 
          />
        )}
        {prediction.cause && (
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {prediction.prevention && (
          <DetailSection 
            icon={FiShield} 
            title="Prevention" 
            content={Array.isArray(prediction.prevention) ? prediction.prevention.join('\n') : prediction.prevention} 
          />
        )}
      ...
      <div className="bg-gradient-to-r from-emerald-600 to-emerald-500 rounded-[2.5rem] p-10 text-center shadow-2xl overflow-hidden relative group">
        <div className="relative z-10">
          ...
          <p className="text-emerald-950 font-bold mb-8 opacity-70">Download the full technical report for offline consultation & sharing.</p>
          <button 
