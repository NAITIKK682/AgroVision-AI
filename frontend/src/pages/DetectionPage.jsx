...
const DetectionPage = () => {
  ...
  const [shouldAutoScan, setShouldAutoScan] = useState(false); // ✅ New flag
  ...
  // ✅ Auto-scan when image captured from camera
  useEffect(() => {
    const autoScanImage = async () => {
      if (!shouldAutoScan || !imageFile) return;
      
      setShouldAutoScan(false); // Reset flag
      setIsLoading(true);
      setError(null);
      ...
      try {
        ...
        if (result.status === 'success' && result.data) {
          const predictionData = result.data.prediction || result.prediction;
          setPrediction(predictionData);
          
          cacheScan({
            ...predictionData,
            image: predictionData.image_url || previewUrl,
          ...
          setError({ 
            message: typeof result.error === 'object' 
              ? result.error.en || result.error 
              : result.error || 'Image validation failed',
            suggestion: result.suggestion
          });
        } else {
          setError({ 
            message: result.message || result.error || 'Diagnostic mismatch detected.' 
          });
        }
      } catch (err) {
        ...
        if (!isOnline) {
          ...
          setError({ 
            message: err.response?.data?.message || 'Critical error during neural analysis.' 
          });
        }
      } finally {
        setIsLoading(false);
    ...
    autoScanImage();
  }, [shouldAutoScan, imageFile, location, isOnline, t, cacheScan, previewUrl]);
  ...
  const handleCameraCapture = (file) => {
    ...
    setError(null);
    setPrediction(null);
    setShouldAutoScan(true); // ✅ Flag to auto-trigger scan
  };
  ...
  const handleScan = async () => {
    ...
    try {
      ...
      if (result.status === 'success' && result.data) {
        // Extract prediction from nested data structure
        const predictionData = result.data.prediction || result.prediction;
        setPrediction(predictionData);
        ...
        cacheScan({
          ...predictionData,
          image: predictionData.image_url || previewUrl,  // Use backend image_url first, fallback to previewUrl
          image_url: predictionData.image_url,  // Store backend URL
          timestamp: new Date().toISOString(),
          location: location,
          scan_id: result.data.scan_id
        });
      } else if (result.status === 'invalid') {
        // Handle validation errors with suggestion
        setError({ 
          message: typeof result.error === 'object' 
            ? result.error.en || result.error 
            : result.error || 'Image validation failed',
