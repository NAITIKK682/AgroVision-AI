...
const HistoryPage = () => {
  ...
  const { cachedScans = [], isLoading: offlineLoading, deleteCachedScan } = useOffline(); // Fallback to empty array
  ...
  // --- Fixed Delete Functionality with Safety Checks ---
  const handleDelete = async (scan, index) => {
    const confirmMsg = lang === 'hi' ? 'क्या आप इस रिकॉर्ड को हटाना चाहते हैं?' : 'Are you sure you want to delete this record?';
    if (!window.confirm(confirmMsg)) return;

    try {
      if (activeTab === 'offline') {
        // Only call if function exists in OfflineContext
        if (deleteCachedScan) {
          // Pass the index to ensure only one record is deleted from the array
          await deleteCachedScan(index);
        } else {
          console.warn("deleteCachedScan function not found in OfflineContext");
        }
      } else {
        // Only call if function exists in api.js
        if (deleteScan) {
          await deleteScan(scan.id);
          setServerHistory(prev => prev.filter(item => item.id !== scan.id));
        } else {
          console.warn("deleteScan function not found in api.js");
        }
      }
    } catch (error) {
              ...
              >
                ...
                <div className="relative flex flex-col sm:flex-row gap-6">
                  ...
                  <div className="relative w-full sm:w-32 h-32 flex-shrink-0 overflow-hidden rounded-2xl border border-white/10">
                    ...
                    <div className={`w-full h-full bg-emerald-900/20 flex items-center justify-center text-3xl ${(scan.image || scan.image_url) ? 'hidden' : ''}`}>🌾</div>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-start mb-2">
                      ...
                      {/* Trash Button - Updated to pass index */}
                      <button 
                        onClick={() => handleDelete(scan, index)}
